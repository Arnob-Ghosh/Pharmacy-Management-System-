...
$(document).on('click', '.add-btn', function (e){
	...
	var productQty = $(this).find('span:last').text()  //getting product total quantity
		...
		type: "GET",
			...
			success: function(response){
				...
				$.each(response.products, function(key, item) {
					...
					if($('#product_table tr > td:first-child:contains('+item.productName+'('+item.strength+'))').length == 0){

                      var m=10;

					  var discountChecked=0;
							var qty_o= $('#product_table tr > td:first-child:contains('+item.productName+'('+item.strength+'))').closest("tr").find("td:eq(2) input[type='number']").val()
							if(isNaN(qty_o) ){
								qty_o=0;
							}
						...
						$('#product_table_body').append('<tr>\
							<td>'+item.productName+'('+item.strength+')</td>\
							<td>'+item.mrp+'</td>\
						...
						var trid = $('#product_table tr > td:first-child:contains('+item.productName+'('+item.strength+'))').closest("tr").index();
						var qty = $('#product_table tr > td:first-child:contains('+item.productName+'('+item.strength+'))').closest("tr").find("td:eq(2) input[type='number']").val()
						var discount = parseFloat($('#product_table tr > td:first-child:contains('+item.productName+'('+item.strength+'))').closest("tr").find("td:eq(4)").text());
                        // var discount=  discountChecked;
						var vat = $('#product_table tr > td:first-child:contains('+item.productName+'('+item.strength+'))').closest("tr").find("td:eq(5)").text()
						var totalProductQty = parseInt($('#product_table tr > td:first-child:contains('+item.productName+'('+item.strength+'))').closest("tr").find("td:eq(14)").text())
						...
						if(add > totalProductQty){
							...
							$('#product_table tr > td:first-child:contains('+item.productName+'('+item.strength+'))').closest("tr").find("td:eq(2) input[type='number']").val(totalProductQty)

							var subTotal = totalProductQty * item.mrp
							$('#product_table tr > td:first-child:contains('+item.productName+'('+item.strength+'))').closest("tr").find("td:eq(3)").text(subTotal.toFixed(2))


							var totalDiscount = parseFloat(discount)
							$('#product_table tr > td:first-child:contains('+item.productName+'('+item.strength+'))').closest("tr").find("td:eq(4)").text(discountChecked)

							var totalVat = parseFloat(vat)
							$('#product_table tr > td:first-child:contains('+item.productName+'('+item.strength+'))').closest("tr").find("td:eq(5)").text(totalVat)
							...
							$('#product_table tr > td:first-child:contains('+item.productName+'('+item.strength+'))').closest("tr").find("td:eq(2) input[type='number']").val(add)
					...
					if(item.available_offer == 'true' && remainder == 0){
						...
						if(freeQty > 0){
							if($('#free_product_table tr > td:contains('+item.productName+'('+item.strength+'))').length == 0){
								...
								alert('Offer Item Added for '+item.productName+'('+item.strength+')!')
								var add =  parseInt($('#free_product_table tr > td:contains('+item.productName+'('+item.strength+'))').closest("tr").find("td:eq(1)").text()) + item.freeQuantity
								$('#free_product_table tr > td:contains('+item.productName+'('+item.strength+'))').closest("tr").find("td:eq(1)").text(add)
							}
						}
					}
		...
		if($(this).closest("tr").find("td:eq(2) input[type='number']").val() > 0){
			...
			if(qty > totalProductQty){
				...
				var total = 0;
                    ...
                    if($('#specialdiscount').val() == 0 || $('#specialdiscount').val() == '' ){
...
$(".specialdiscount").change(function(){
	...
	if(specialDiscountAmount > total){
		...
		var grantTotalAfterDiscount = parseFloat(total - (specialDiscountAmount+prevDiscount))
