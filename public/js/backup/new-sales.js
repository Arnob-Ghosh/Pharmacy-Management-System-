...
function productAddToTable() {
	...
	if(productId != 'option_select' && quantity.length != 0  && mrp.length != 0){
	    ...
	    $('#grandtotal').val(((parseFloat($('#totalX').val()) - parseFloat($('#discount').val())) + parseFloat($('#tax').val())).toFixed(2) );
...
function OrderSubmitToServer() {
    ...
    if(productList.length>0){
    	if($('#clientid').val() != 'option_select' && $('#store').val() != 'option_select' && $('#pos').val() != null){
    		submitOrder(orders);
    	}else{
