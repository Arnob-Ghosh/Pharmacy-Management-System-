$(document).ready(function () {
	...
	$.ajax({
		...
		success: function(response){
			...
			//PURCHASE --------------------------------------------------------
			var todayPurchase = parseFloat(response.todayPurchase)
			var totalMonthPurchase = parseFloat(response.totalMonthPurchase)
			...
			if(response.todayPurchase == null){
				...
				$('#todayPurchase').text(todayPurchase.toFixed(2));
			}

			if(response.totalMonthPurchase == null){
				$('#monthPurchase').text(zero.toFixed(2));
			}else{
				$('#monthPurchase').text(totalMonthPurchase.toFixed(2));
			}
