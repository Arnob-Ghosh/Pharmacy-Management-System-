...
class SupplierPaymentController extends Controller
{
    public function index()
    {
        ...
        return view('supplier-payment/supplier-payment', [
    ...
    public function getHeadCode($getHeadCode)
    {
        $data = Supplier::find($getHeadCode);
    ...
    public function getBalanceByHeadCode($getHeadCode)
    {
        $data = Transaction::where('head_code', $getHeadCode)->latest()->first();
        if($data !=''){
            return response()->json([
                'status' => 200,
                'data' => $data
