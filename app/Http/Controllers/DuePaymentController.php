...
class DuePaymentController extends Controller
{
    ...
    public function list(Request $request, $subscriberId, $storeId){
        ...
        //             // ->take(5)
        //             ->get();
        $clients=Client::select('clients.*')->join('stores','stores.subscriber_id','clients.subscriber_id')
            ->where([['stores.subscriber_id', $subscriberId],['stores.id', $storeId]])->groupBy("clients.id")->get();
        ...
        foreach($clients as $client){
            ...
            if($client != ''){
                ...
                // $lastBalance = DB::table('deposits')->where('client_id', $clientId)->latest()->first();
                $lastBalance = DB::table('deposits')->where([
                    ['client_id', $clientId],
                    ['subscriber_id', $subscriberId],
                    ['store_id', $storeId],
                ...
                $totalStoreDue = $totalStoreDue +  $balance;
