        ...
        if($checkReturn->isEmpty()){
            ...
            foreach($request->productList as $product){
                ...
                if((int)$product['returnQty'] > 0){
                    ...
                    $purchase_return->return_qty = (int)$product['returnQty'];
                    ...
                    if((int)$request->storeId == 0){
                        ...
                        foreach($inventories as $inventoryX){
                            ...
                            if($onHand > 0 && $onHand >= (int)$product['returnQty']){
                                $inventory = Inventory::find($inventoryX->id);
                                $inventory->onHand = $onHand - (int)$product['returnQty'];
                                $inventory->productIncoming =  $productIncoming - (int)$product['returnQty'];
                                $boolX = 1;
                            }
                        }
                        ...
                        foreach($store_inventories as $store_inventoryX){
                            ...
                            if($onHand > 0 && $onHand >= (int)$product['returnQty']){
                                ...
                                $store_inventory->productIncoming =  $productIncoming - (int)$product['returnQty'];
