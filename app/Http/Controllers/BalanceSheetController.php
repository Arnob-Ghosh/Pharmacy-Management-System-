...
class BalanceSheetController extends Controller
{
    ...
    public function data(Request $request){
        ...
        $loanReceivables = DB::table("transactions")
                // ->join("chart_of_accounts", "head_code", "chart_of_accounts.head_code")
                ->select(DB::raw("SUM(debit) as totalDebit"), DB::raw("SUM(credit) as totalCredit"), 'head_code', 'head_name', 'head_type')
                ->where([['subscriber_id', Auth::user()->subscriber_id], ['head_code', 1010102]])
        ...
        $serviceReceivables = DB::table("transactions")
                // ->join("chart_of_accounts", "transactions.head_code", "chart_of_accounts.head_code")
                ->select(DB::raw("SUM(debit) as totalDebit"), DB::raw("SUM(credit) as totalCredit"), 'head_code', 'head_name', 'head_type')
                ->where([['subscriber_id', Auth::user()->subscriber_id], ['head_code', 1010103]])
                ->orderBy('head_code', 'asc')
                ->groupBy("head_code", "head_name", "head_type")
                // ->take(5)
                ->get();
        ...
        $accountReceivableHeadName = 'Account Recivable';
        ...
        $cashInHandCashs = DB::table("transactions")
            // ->join("chart_of_accounts", "transactions.head_code", "chart_of_accounts.head_code")
            ->select(DB::raw("SUM(debit) as totalDebit"), DB::raw("SUM(credit) as totalCredit"), 'head_code', 'head_name', 'head_type')
            ->where([['subscriber_id', Auth::user()->subscriber_id], ['head_code', 1010202]])
            ->orderBy('head_code', 'asc')
            // ->groupBy("transactions.head_code", "transactions.head_name", "transactions.head_type")
            // ->take(5)
            ->get();
        ...
        $pettyCashs = DB::table("transactions")
            // ->join("chart_of_accounts", "transactions.head_code", "chart_of_accounts.head_code")
            ->select(DB::raw("SUM(debit) as totalDebit"), DB::raw("SUM(credit) as totalCredit"), 'head_code', 'head_name', 'head_type')
            ->where([['subscriber_id', Auth::user()->subscriber_id], ['head_code', 1010203]])
            ->orderBy('head_code', 'asc')
            // ->groupBy("head_code", "head_name", "head_type")
            // ->take(5)
            ->get();
        ...
        $inventories = DB::table("transactions")
            // ->join("chart_of_accounts", "transactions.head_code", "chart_of_accounts.head_code")
            ->select(DB::raw("SUM(debit) as totalDebit"), DB::raw("SUM(credit) as totalCredit"), 'head_code', 'head_name', 'head_type')
            ->where([['subscriber_id', Auth::user()->subscriber_id], ['head_code', 10201]])
            ->orderBy('head_code', 'asc')
            ->groupBy("head_code", "head_name", "head_type")
            // ->take(5)
            ->get();
        ...
        $tassets = DB::table("transactions")
            // ->join("chart_of_accounts", "transactions.head_code", "chart_of_accounts.head_code")
            ->select(DB::raw("SUM(debit) as totalDebit"), DB::raw("SUM(credit) as totalCredit"), 'head_code', 'head_name', 'head_type')
            ->where([['subscriber_id', Auth::user()->subscriber_id], ['head_code', 103]])
            ->orderBy('head_code', 'asc')
            ->groupBy("head_code", "head_name", "head_type")
            // ->take(5)
            ->get();
        ...
        $employeePayables = DB::table("transactions")
            // ->join("chart_of_accounts", "transactions.head_code", "chart_of_accounts.head_code")
            ->select(DB::raw("SUM(debit) as totalDebit"), DB::raw("SUM(credit) as totalCredit"), 'head_code', 'head_name', 'head_type')
            ->where([['subscriber_id', Auth::user()->subscriber_id], ['head_code', 50102]])
            ->orderBy('head_code', 'asc')
            ->groupBy("head_code", "head_name", "head_type")
            // ->take(5)
            ->get();
        // log::info($employeePayables->toarray());
        ...
        $supplierPayables = DB::table("transactions")
            // ->join("chart_of_accounts", "transactions.head_code", "chart_of_accounts.head_code")
            ->select(DB::raw("SUM(debit) as totalDebit"), DB::raw("SUM(credit) as totalCredit"), 'head_code', 'head_name', 'head_type')
            ->where([['subscriber_id', Auth::user()->subscriber_id], ['head_code', 50103]])
            ->orderBy('head_code', 'asc')
            ->groupBy("head_code", "head_name", "head_type")
            // ->take(5)
            ->get();
        ...
        $taxPayables = DB::table("transactions")
            // ->join("chart_of_accounts", "transactions.head_code", "chart_of_accounts.head_code")
            ->select(DB::raw("SUM(debit) as totalDebit"), DB::raw("SUM(credit) as totalCredit"), 'head_code', 'head_name', 'head_type')
            ->where([['subscriber_id', Auth::user()->subscriber_id], ['head_code', 50104]])
            ->orderBy('head_code', 'asc')
            ->groupBy("head_code", "head_name", "head_type")
            // ->take(5)
            ->get();
        ...
        $nonCurrentLiabilities = DB::table("transactions")
            // ->join("chart_of_accounts", "transactions.head_code", "chart_of_accounts.head_code")
            ->select(DB::raw("SUM(debit) as totalDebit"), DB::raw("SUM(credit) as totalCredit"), 'head_code', 'head_name', 'head_type')
            ->where([['subscriber_id', Auth::user()->subscriber_id], ['head_code', 502]])
            ->orderBy('head_code', 'asc')
            ->groupBy("head_code", "head_name", "head_type")
            // ->take(5)
            ->get();
    ...
    ->select(
        DB::raw("SUM(transactions.debit) as totalDebit"),
        DB::raw("SUM(transactions.credit) as totalCredit"),
        'transactions.head_code',
        'transactions.head_name',
        'transactions.head_type'
    )
    ->where([
        ['transactions.subscriber_id', Auth::user()->subscriber_id], 
        ['chart_of_accounts.parent_head_level', 2]
    ])
    ->orderBy('transactions.head_code', 'asc')
    ->groupBy("transactions.head_code", "transactions.head_name", "transactions.head_type")
    ->get();
    ...
    public function dateWiseData(Request $request)
    {
        ...
        $cashInHandCashs = DB::table("transactions")
            ->join("chart_of_accounts", "transactions.head_code", "chart_of_accounts.head_code")
            ->select(DB::raw("SUM(transactions.debit) as totalDebit"), DB::raw("SUM(transactions.credit) as totalCredit"), 'transactions.head_code', 'transactions.head_name', 'transactions.head_type')
            ->where([['transactions.subscriber_id', Auth::user()->subscriber_id], ['chart_of_accounts.head_code', 1010202]])
            ->whereBetween('transactions.transaction_date', [$start_date, $end_date])
            ->orderBy('transactions.head_code', 'asc')
            ->groupBy("transactions.head_code", "transactions.head_name", "transactions.head_type")
            // ->take(5)
        ...
        $totalBalanceCashandCashEquivalent = $totalDebitCashandCashEquivalent - $totalCreditCashandCashEquivalent;
        // Log::info($totalBalanceCashandCashEquivalent);
        $cashandCashEquivalent = 'Cash & Cash Equivalent';
        ...
        $inventories = DB::table("transactions")
            ...
            // ->take(5)
            ->toSql();
            // Log::info( Auth::user()->subscriber_id);
        $totalCreditInventory = 0;
        $totalDebitInventory = 0;
        foreach ($inventories as $inventory) {
            $totalCreditInventory = $totalCreditInventory + $inventory->totalCredit;
        ...
        $totalAsset = $totalCurrentAsset + $totalNonCurrentAsset;
