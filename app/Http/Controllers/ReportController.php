    ...
    public function onLoad(Request $request){
        $data = DB::table("ordered_products")
             ...
             $data1 = DB::table("ordered_products")
                ->select(DB::raw("SUM(totalPrice) as gt"), "productName")
                ->where('subscriber_id', '=', Auth::user()->subscriber_id )
                ->groupBy("productName")
                ->orderBy("gt","desc")
                ->get();
            ...
            $data2 = DB::table("ordered_products")
                ->select(DB::raw("SUM(grandTotal) as gt"), DB::raw("SUM(quantity) as qty"), DB::raw("SUM(totalDiscount) as td"), DB::raw("SUM(totalTax) as tt"), DB::raw("SUM(totalPrice) as tp"),"productName", "price")
                ->where('subscriber_id', '=', Auth::user()->subscriber_id )
                ->groupBy("productName")
                ->get();
        //($data2);
